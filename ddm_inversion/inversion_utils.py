...
def inversion_forward_process(model, x0, 
    ...
    for t in op: # t decreasing since op is also decreasing: 991 -> 981 -> 971 -> ... -> 1
        ...
        if not eta_is_zero:
            xt = xts[idx+1][None] # xts[100] -> xts[99] -> ... -> xts[0] or x_noisy -> x_clean
...
def reconstruction_from_inv_noise(model, prompt, xts, zs, noise_pred_list, num_inference_steps, etas, cfg_scale=3.5):
    ...
    i = len(xts) - 1
    j = 0
    for t in op: # noise 991 to noise 1
        idx = num_inference_steps-t_to_idx[int(t)]-1 # 100 - 0 - 1 -> 100 - 1 - 1 -> ... -> 100 - 99 - 1
        with torch.no_grad():
            out = model.unet.forward(x_last, timestep=t, encoder_hidden_states=uncond_embedding)
            if not prompt=="":
                cond_out = model.unet.forward(x_last, timestep=t, encoder_hidden_states=text_embeddings)
        ...
        if not prompt=="":
            ...
            noise_pred = out.sample
        print(f"Step {i}: noise_pred: {torch.mean(torch.abs(noise_pred_list[j] - noise_pred))}, latent: {torch.mean(torch.abs(xts[i][None] - x_last))}")
        i-=1
        j+=1
        
        prev_timestep = t - model.scheduler.config.num_train_timesteps // model.scheduler.num_inference_steps
        pred_original_sample = (x_last - (1-alpha_bar[t])  ** 0.5 * noise_pred ) / alpha_bar[t] ** 0.5
        alpha_prod_t_prev = model.scheduler.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else model.scheduler.final_alpha_cumprod
        variance = get_variance(model, t)
        pred_sample_direction = (1 - alpha_prod_t_prev - etas[idx] * variance ) ** (0.5) * noise_pred
        mu_xt = alpha_prod_t_prev ** (0.5) * pred_original_sample + pred_sample_direction
        x_last = mu_xt + (etas[idx] * variance ** 0.5 )*zs[idx]
    return x_last

def denoise_from_inv_noise(model, src_prompt, tgt_prompt, xts, zs, num_inference_steps, etas, skip, cfg_scale_src=3.5, cfg_scale_tar=8.0):
    etas = [etas]*model.scheduler.num_inference_steps
    timesteps = model.scheduler.timesteps.to(model.device)
    t_to_idx = {int(v):k for k,v in enumerate(timesteps)}
    alpha_bar = model.scheduler.alphas_cumprod
    op = tqdm(timesteps)
    x_last = xts[-1][None]
    src_embedding = encode_text(model, src_prompt)
    tgt_embedding = encode_text(model, tgt_prompt)
    uncond_embedding = encode_text(model, "")
    for i, t in enumerate(op):
        idx = num_inference_steps-t_to_idx[int(t)]-1
        with torch.no_grad():
            out = model.unet.forward(x_last, t, encoder_hidden_states=uncond_embedding)
            if i < skip:
                cond_out = model.unet.forward(x_last, timestep=t, encoder_hidden_states=src_embedding)
                noise_pred = out.sample + cfg_scale_src * (cond_out.sample - out.sample)
            else:
                cond_out = model.unet.forward(x_last, timestep=t, encoder_hidden_states=tgt_embedding)
                noise_pred = out.sample + cfg_scale_tar * (cond_out.sample - out.sample)
