...
from src.actor_critic import UnetActorDiscrete, UnetDownSampling
from src.dataset import AutoEditDataset
from src.sft_finetuning import SFTFineTuningHead
...
def main(args):
    ...
    pipeline.unet = unet
    model = UnetActorDiscrete(1280*2, 768)
    ...
    pipeline.to("cuda")
    trainer = SFTFineTuningHead(args, cfg, model, train_dataset, pipeline, inv_scheduler)
    # trainer.fit()
    # trainer.test_generation_trajectory()
    trainer.test_sft_finetuning_phase()
